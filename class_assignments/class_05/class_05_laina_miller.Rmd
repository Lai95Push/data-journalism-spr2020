...
```{r}
#storing buyer_addresses, buyer_totals, buyer_annual_by_year, and state_population_per_year csv files as objects with same names (minus .csv)
...
#I looked at the summaries in the previous code block. My guess is that there are more records in buyer_annual_by_year because it records each buyer for each year, creating repeats of records of buyers if they bought during more than one year. In contrast, buyer_totals is just counting the number of buyers and how many pills they bought overall.
#buyer_addresses probably lists all of the pharmacies that have multiple locations, and also pharmacies that didn't ship any opiates.
...
#used inner join to make buyer_totals_ids, used a summary to get a look. Then moved total_pills and buyer_name to the front of the table and sorted total_pills from highest to lowest.

buyer_totals_ids <- buyer_totals %>%
  inner_join(buyer_addresses, by="buyer_dea_no")
buyer_totals_ids %>%
  summary()
buyer_totals_ids %>%
  select(total_pills, buyer_name, everything()) %>%
  arrange(desc(total_pills))


#VA CONSOLIDATED MAIL OUTPATIENT PHARMACY is a national-scale VA mail-order pharmacy, meaning that it is serving a much larger population than other, local, pharmacies.
...
buyer_completes <- buyer_addresses %>%
  inner_join(buyer_annual_by_year, by="buyer_dea_no")

buyer_completes %>%
  filter(buyer_state.x == "MD" & buyer_state.y == "MD" & buyer_bus_act.y == "PRACTITIONER" & buyer_bus_act.x == "PRACTITIONER" & year == "2010") %>%
  arrange(desc(dosage_unit))

#Rakesh K. Mathur, MD had 337,450 pills, 124,650 pills more than the next-highest doctor in MD in 2010. He has 6 disciplinary actions from MD and 1 pending charge, and his license to practice is suspended. The address in this data matches the address he had that is listed in some of the disciplinary action records.
