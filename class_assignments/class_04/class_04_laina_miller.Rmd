...
```{r}
#creating object called 'baltimore' with tsv file
...
```{r}
#open obj baltimore in a new window tab
#View(baltimore)
```
...
```{r}
#There are 42 columns and 290,908 rows
#Columns stored as "dbl" that don't need to be include "REPORTER_ZIP", "BUYER_ZIP", "DRUG_CODE", and "TRANSACTION_ID"
#I don't think that there are any columns that ought to be stored as "dbl" that are not
#Missing values are represented with "null"

#running glimpse and summary
glimpse(baltimore)
summary(baltimore)
```
...
#Newcare Home Health Services, located in the 3400 block of Sinclair Lane, Baltimore, received over 6 million pills' worth of opiates between 2006 and 2012. In late 2006, several employees of NHHS were indicted for illegal sale of schedule III drugs. In 2007, the DEA revoked their registration as a pharmacy. In 2009, the MD State Board of Pharmacy revoked the Pharmacist's License belonging to one of these employees, one Steven Sodipo.
...
baltimore %>%
  ...
  arrange(desc(total_pills))
#McKesson Corporation; 58,267,330; their corporate HQ is in Texas
```
...
#Professional Pharmacy Service, address is Sinai Hospital, had 6,254 shipments. It's a hospital, and probably ordered pills on a need-by basis, meaning lots of smaller shipments.
