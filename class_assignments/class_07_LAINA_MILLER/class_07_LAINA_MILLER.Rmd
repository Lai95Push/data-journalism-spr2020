...
arcos_county_population_pills_per_year <- arcos_county_pills_per_year %>%
  full_join(arcos_county_population_per_year, by = c("countyfips", "year", "buyer_county","buyer_state"))
```

```{r}
# Step 4: Filter the pills population table just for West Virginia and create the pills per person metric. 

wv_county_pills_population_per_year <- arcos_county_population_pills_per_year %>%
...
geo_wv_pills_pop_yr <- county_geodata %>%
  inner_join(wv_county_pills_population_per_year, by = c("GEOID", "NAME"))
```
...
geo_wv_pills_pop_yr %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill = "Pills per person", title = "Pills per person increases from 2006 to 2008, then decreases from 2009 to 2014", subtitle = "Pills per person in West Virginia, 2006-2014", caption = "Source: ARCOS database and U.S. Census") +
...
```{r}
#Filter arcos_county_population_pills_per_year for WV and KY

wv_ky_pills_population_per_year <- arcos_county_population_pills_per_year %>%
...
```{r}
#joining wv_ky_pills_population_per_year with county_geodata

geo_wv_ky_pills_pop_per_year <- county_geodata %>%
  inner_join(wv_ky_pills_population_per_year, by = c("GEOID" = "countyfips"))

#making the facet map

geo_wv_ky_pills_pop_per_year %>%
  ggplot(aes(fill = pills_per_person)) +
  facet_wrap(~year, nrow=2) +
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill = "Pills per person", title = "Highest rate of pills per person in 2008, at the intersection of Kentucky and West Virginia", subtitle = "Pills per person in West Virginia, Kentucky, 2006-2009. Pills per person rises to peak from 2006 to 2008", caption = "Source: ARCOS database and U.S. Census") +
  theme(legend.position = "bottom") +
  scale_fill_viridis_c(option = "magma", labels = comma)
