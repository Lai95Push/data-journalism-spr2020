...
ggplot(maryland_pills_per_year) +
  geom_bar(stat = "identity", aes(year, dosage_unit), fill="red") +
  labs(x = "Year", y = "Total Pills", title = "Number of opioid pills shipped to Maryland increased steadily \nfrom 2006 to 2011, then dropped slowly", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014)) +
  scale_y_continuous(labels = comma, breaks = c(0, 25000000, 50000000, 75000000, 100000000, 125000000, 150000000, 175000000, 200000000))

#TASK 2 ANSWER: It is not the same. Baltimore City had a massive drop from 2006 to 2007, then rose steadily until 2012, before falling very slightly and leveling out. Overall, Maryland's pills increased steadily until 2011, then dropped slowly until 2014.
...
```{r}
us_pills_per_year <- arcos_county_pills_per_year %>%
...
```{r}
us_pills_per_2012 <- us_pills_per_year %>%
  select(buyer_state, year, dosage_unit)%>%
  filter(year == "2012")
```

```{r}
us_pills_per_2012b <- us_pills_per_2012 %>%
  group_by(buyer_state) %>%
  summarize(dosage_unit = sum(dosage_unit))
...
```{r}
us_population_2012 <- state_population(key = key) %>%
  clean_names() %>%
  filter(year == "2012") %>%
  select(buyer_state, population)

us_2012 <- us_pills_per_2012b %>%
  inner_join(us_population_2012, by=("buyer_state")) %>%
  select(buyer_state, population, dosage_unit)
...
ggplot(us_2012) + 
  ...
  labs(x="2012 Population", y="Total pills in 2012", title = "Five states with top pill-per-person ratios for opiates", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(labels = comma, breaks = c(0, 5000000, 10000000, 15000000, 20000000, 25000000, 30000000, 35000000)) +
  scale_y_continuous(labels = comma, breaks = c(0, 100000000, 200000000, 300000000, 400000000, 500000000, 600000000, 700000000, 800000000, 900000000, 1000000000)) +
  geom_smooth(aes(population, dosage_unit), method = "lm", se = FALSE) +
  geom_text_repel(aes(population, dosage_unit, label=buyer_state), 
                  subset(us_2012, per_person > 64),
                  arrow = arrow(length = unit(0.02, "npc")), box.padding = 1, colour = "red") +
  
ggsave("us_pills_per_state_pop_2012.png", plot = last_plot())

#Top five are Tennesee (66.71), Kentucky (72.14), Alabama (65.11), Oklahoma (65.77), West Virginia (71.55)
```
### Q3 through infinity
