...
```{r}
# Using glimpse to view mingo data
glimpse(mingo)
```

```{r}
#Using summary and view to get a summarized view of data and open new tab for data
summary(mingo)
#view(mingo)
```
...
```{r}
#making nice complete address column in mingo_working
mingo_working <- mingo %>%
...
```{r}
#using if_else to make a new column dividing into dist states as in wv or outside of wv
mingo_working <- mingo %>%
...
mingo %>%
  ...
  summarise(shipments = n(),
            ...
            min_pills_in_shipment = min(dosage_unit),
            max_pills_in_shipment = max(dosage_unit),
            med_pills_in_shipment = median(dosage_unit)
            ) %>%
  arrange(desc(max_pills_in_shipment))
```
...
```{r}
#The most significant combo is Actavis Pharma to Miami-Luken to Tug Valley Pharmacy, which yielded 4,954,200 pills
mingo %>%
...
mingo %>%
  group_by(combined_labeler_name, reporter_name, reporter_dea_no, buyer_name, buyer_dea_no) %>%
  summarise(pills_per_pharmacy = sum(dosage_unit)) %>%
  arrange(desc(pills_per_pharmacy))
...
You're more interested in looking at the company-to-company-to-company pipeline, instead of the location-to-location pipeline.  In order to proceed with your analysis, you'll need to think of a way to clean the data so that all three Cardinal Health facilities have the same name. Otherwise they won't group correctly.  
...
```{r}
#you should be able to use case_when() to select for all of the names under reporter_name that contain "Cardinal Health" and, in a new column, label them all as "Cardinal Health". this will allow you to then sort by the new column, instead of the old one with multiple names for one company
