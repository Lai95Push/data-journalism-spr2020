...
```{r}
#grouping into 5 buckets based on death-rate (age-adjusted), then summarizing to see if the avg pills per person is also higher with higher death rates.
...
pills_population_income %>%
  ...
  group_by(quintile) %>%
  summarise(median_household_income = mean(median_household_income), average_pills_per_person = mean(average_pills_per_person))
...
ggplot(pills_population_income) +
  geom_point(aes(median_household_income, average_pills_per_person)) +
  geom_smooth(aes(median_household_income, average_pills_per_person), method = "lm", se = FALSE) +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Median household income per county", y = "Average pills per person per county", caption = "Source DEA ARCOS database, via Washington Post, and the CDC")
...
#Making a table for only correlation, then find for r
...
#Previous calculation shows a negative correlation with an r value of -0.3
correlation_income_pills_2 <- pills_population_income %>%
  ungroup() %>%
  select(median_household_income, average_pills_per_person) 
#Calculation statistical significance:

cor.test(correlation_income_pills_2$median_household_income, correlation_income_pills_2$average_pills_per_person)


#According to previous explanation of this correlation test, returning with a p-value of <2.2e-16 means that this is a statistically significant correlation. It appears that there may be a story in here about the correlation between low-income areas and opioid quantities.
