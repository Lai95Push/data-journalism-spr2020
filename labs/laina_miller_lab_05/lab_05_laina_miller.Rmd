...
```{r}
#loading tidyverse, janitor, and lubridate
...
```{r}
#changing to mdy aka month-day-year to make date into dates
mingo <- mingo %>%
...
```{r}
#extracting year info from transaction_date and new column under transaction_year, then reorder for transaction_date and transaction_year to be the first two columns
mingo <- mingo %>%
...
```{r}
#grouping by year and seeing amounts of individual pills shipped per year
mingo %>%
  group_by(transaction_year) %>%
  summarise(dosage_unit = n())
#the amounts of pills increased by 1-1.5 thousand pills until 2008, teetered in the 7-7.5 thousand range from 2008-2009, and then began decreasing visibly in 2010, until the 2012 amount was less than the 2006 amount
