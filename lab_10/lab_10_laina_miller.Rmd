...
We're going to clean these things up one-by-one. Failure to do this will mean problems down the road.  
...
opioid_scrape_working <- opioid_scrape %>%
  ...
  mutate(opioid_involved_overdose_deaths_100_000_persons1_2017 = na_if(opioid_involved_overdose_deaths_100_000_persons1_2017, "*"))
...
opioid_scrape_working <- opioid_scrape %>%
  ...
  filter(row_number() != 36) %>%
  mutate(opioid_involved_overdose_deaths_100_000_persons1_2017 = na_if(opioid_involved_overdose_deaths_100_000_persons1_2017, "*")) %>%
  rename(death_rate = opioid_involved_overdose_deaths_100_000_persons1_2017) %>%
  rename(rx_rate = opioid_prescriptions_100_persons2_2017)
...
medicare_opioids_working <- medicare_opioids %>%
  clean_names() %>%
  remove_empty_rows() %>%
  filter(row_number() != 25) %>%
  mutate(medicare_opioid_prescribing_rate = na_if(medicare_opioid_prescribing_rate, "NO VALUE PROVIDED")) %>%
  mutate(medicare_urban_opioid_prescribing_rate = na_if(medicare_urban_opioid_prescribing_rate, "NO VALUE PROVIDED")) %>%
  mutate(medicare_rural_opioid_prescribing_rate = na_if(medicare_rural_opioid_prescribing_rate, "NO VALUE PROVIDED")) %>%
  mutate(medicare_urban_opioid_prescribing_rate = str_replace(medicare_urban_opioid_prescribing_rate, "ZERO", "0")) %>%
  separate(county, into = c("county", "state"), sep = ",") %>%
  mutate(county = tolower(county)) %>%
