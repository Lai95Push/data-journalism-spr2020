...
```{r}
# In the not_in_population table, there are several records for each county, and many of them don't have a countyfips number. You also have several locations that are not in the not-in-pills table.
```
...
```{r}
# Define API Key
census_api_key("bead523682d2fc9ff7a82f0869f36a900cfa3cc1")
...
#geo_pills_population_state_2009 %>%
  ...
  #geom_sf(lwd = 0) +
  #theme_map() +
  #labs(fill = 'Pills Per Person', title = "West Virginia and Kentucky have the most pills per person",  subtitle = "2009 population and pills per person data, U.S. Census and ARCOS", caption = "Sources: ARCOS data and U.S. Census ACS") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills per person increases, particularly in the Midwest, Nevada, Oregon", subtitle = "Rate of increase between 2006 and 2012, according to ARCOS data", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
#geo_pills_pop_county_2009 <- pills_population %>%
  #filter(year==2009) %>%
  #group_by(countyfips, buyer_county, buyer_state) %>%
  #summarise(pills_per_person = dosage_unit/population)
#geo_pills_pop_county_2009 <- county_geodata_shifted %>%
  #left_join(pills_population_county_2009, by=c("countyfips"))

```

```{r}
#mapview(geo_pills_pop_county_2009, zcol = "pills_per_person", legend = TRUE)


#Higher pills-per-person rates spread from Appalachia into Tennessee and the rest of the southern Midwest, but there is also a higher pill-per-person rate on the West Coast from Oregon down into Northern California and Nevada.
```
...
```{r}
# map code here
#maryland_county_geodata_shifted <- county_geodata_shifted %>%
  #mutate(statefips = str_sub(countyfips, 1,2)) %>%
  #filter(statefips == "24")
#md_county_geodata_shifted <- maryland_county_geodata_shifted %>%
  #left_join(pills_population)
```

```{r}
#md_county_geodata_shifted %>%
  #summarise()
